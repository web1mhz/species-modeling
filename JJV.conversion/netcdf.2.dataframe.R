...
nc = open.ncdf('climgen_output.nc')	#open a connection to the netcdf of interest
	lon = get.var.ncdf(nc,'LONGITUDE')	#get a list of the longitudes
    lat = get.var.ncdf(nc,'LATITUDE')	#get a list of the latitudes
	month = get.var.ncdf(nc,'MONTH')	#get a list of the months
	...
	for (voi in vars) { cat(voi,'...\n') #cycle through each of hte variables
		tdata = get.var.ncdf(nc,voi) #read in the data array for the variable of interses
		for(ii in 1:length(month)) { out[,paste(voi,'_',sprintf('%02i',ii),sep='')] = as.vector(tdata[ii,,]) } #append the data to out
	}
