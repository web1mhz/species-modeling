...
#open a link to the first file

for (ii in 1:length(bins)) {
	#define the subset of lines for this shell script
	if (!ii == length(bins)) { nmin = bins[ii];nmax = bins[ii+1] } else { nmin = bins[ii];nmax = length(bins) }
	...
	close(zz)
	#submit the job
	system(paste('qsub prep',sprintf('%05i',nmin),'.sh',sep=''))
}
